...
>>>>>>> main
	...
	private void checkDestinatonTile(Tile destinationTile) {
		...
		if(destinationTile.getColor().equals(Color.yellow))
			...
			while(!leaglTile) {
				do {
					System.out.println("Please enter the tile coordinates [format: (row,col)] that you want the soldier to return to:");
			...
			System.out.println("You had won a one more turn with the same soldier. [indexes: (" + destinationTile.getRows() + "," + destinationTile.getCols() + ")");
			myTimer.reset();
			Scanner scanner = new Scanner(System.in);
			String input;
			do {
